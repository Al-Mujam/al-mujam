<template>
  ...
  <ul>
    ...
    <li><NuxtLink class="opacity-70 hover:opacity-100 transition-all duration-300" :to="{name: 'blog-resources'}">{{ translations.find(t => t.key === 'blog_resources').value[locale] }}</NuxtLink></li>
  </ul>
</div>
             </div>
...
<section v-if="activeTab === 'blogs'" class="container mx-auto px-4 md:px-20 pb-5 lg:px-10 max-w-[1280px]">
...
useSeoMeta({
    title:  () => `${translations.value.find(t => t.key === 'blog_resources').value[locale.value]} - ${settings.value.find(setting => setting.key === 'app_name').value[locale.value]}`,
    description: () => settings.value.find(setting => setting.key === 'blog_resources_page_seo_desc').value[locale.value],
    ogTitle: () => `${translations.value.find(t => t.key === 'blog_resources').value[locale.value]} - ${settings.value.find(setting => setting.key === 'app_name').value[locale.value]}`,
    ogDescription: () => settings.value.find(setting => setting.key === 'blog_resources_page_seo_desc').value[locale.value],
    ogImage: '/brand/seo_image.png',
    twitterTitle: () => `${translations.value.find(t => t.key === 'blog_resources').value[locale.value]} - ${settings.value.find(setting => setting.key === 'app_name').value[locale.value]}`,
})
